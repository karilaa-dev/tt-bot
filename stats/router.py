    ...
    try:
        ...
        if graph_time == "daily":
            ...
            async with await get_session() as db:
                ...
                period = (
                    first_record if first_record is not None else time_now - 86400 * 365
                )
            depth = "%Y-%m-%d"
        ...
        await temp.edit_text(
            "<code>Error generating graph. Please try again later.</code>"
        )
...
async def stats_overall(call: CallbackQuery):
    ...
    try:
        ...
        logging.debug("Stats overall message unchanged")
...
async def stats_user(call: CallbackQuery, state: FSMContext):
    msg = await call.message.edit_text(
        "🔎Enter user id", reply_markup=stats_return_keyboard
    )
...
async def stats_user_search(message: Message, state: FSMContext):
    ...
    if not user:
        ...
        keyb = InlineKeyboardBuilder()
        keyb.button(
            text="📥Download video history", callback_data=f"user:{user.user_id}"
        )
...
async def stats_user_download(call: CallbackQuery):
    ...
    if not videos:
        ...
        await call.message.answer_document(
            BufferedInputFile(file.read().encode(), f"user_{user_id}.csv"),
...
async def stats_other(call: CallbackQuery):
    ...
    try:
        ...
        logging.debug("Stats referral message unchanged")
        await call.answer("⚠Nothing to update")
...
async def stats_other(call: CallbackQuery):
    ...
    try:
        ...
        logging.debug("Stats other message unchanged")
        await call.answer("⚠Nothing to update")
...
async def stats_detailed(call: CallbackQuery):
    await call.message.edit_text(
        "<b>📊 Select Chat Type</b>", reply_markup=stats_detailed_keyboard()
    )
...
async def stats_type_selection(call: CallbackQuery):
    ...
    await call.message.edit_text(
        "<b>📊 Select Time Period</b>", reply_markup=stats_time_keyboard(chat_type)
    )
...
async def stats_callback(call: CallbackQuery):
    ...
    await call.message.edit_text(
        await bot_stats(group_type, stats_time), reply_markup=keyb
    )
    await call.answer()
