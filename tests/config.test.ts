...
const names = ["BOT_TOKEN", "DB_URL", "TT_SCRAP_API_KEY", "ADMIN_IDS", "SECOND_IDS", "TG_SERVER", "TT_SCRAP_BASE_URL", "TT_SCRAP_REQUEST_TIMEOUT_SECONDS", "TT_SCRAP_INSTAGRAM_DELIVERY_PATH", "MAX_USER_QUEUE_SIZE", "MAX_GROUP_QUEUE_SIZE", "MAX_ACTIVE_JOBS", "DB_POOL_SIZE"];
...
describe("loadConfig", () => {
  test("normalizes the legacy asyncpg URL and combines administrator IDs", () => {
    const config = loadConfig();
    expect(config.databaseUrl).toBe("postgresql://u:p@db/x");
    expect(config.adminIds).toEqual(new Set([1, 2]));
