...
for lang_name in locale["langs"]:
    lang_keyboard.button(
        text=locale[lang_name]["lang_name"], callback_data=f"lang/{lang_name}"
    )
...
async def inline_lang(callback_query: CallbackQuery):
    ...
    lang = callback_query.data.removeprefix("lang/")
    ...
    try:
        ...
        await bot.edit_message_text(
            text=locale[lang]["lang"], chat_id=chat_id, message_id=msg_id
        )
    except TelegramBadRequest:
        logging.debug("Language message already updated or unchanged")
    except Exception as e:
        logging.error(f"Failed to update user language: {e}")
