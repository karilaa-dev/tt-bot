...
export function createInlineSlideshow(api: Api, inlineMessageId: string, fileIds: string[], lang: Language, sourceLink: string, identity: { userId: number; username?: string; fullName: string }, likes?: number | null, views?: number | null): InlineKeyboardMarkup {
  if (!fileIds.length) throw new Error("Slideshow delivery returned no Telegram photo file IDs");
  const old = sessions.get(inlineMessageId);
  if (old) clearTimeout(old.timer);
  const session: SlideshowSession = {
    fileIds, currentIndex: 0, lang, sourceLink, userId: identity.userId, fullName: identity.fullName,
  ...
  return navigationKeyboard(0, fileIds.length, likes, views);
}
...
export function registerInlineSlideshowHandlers(bot: Bot<BotContext>): void {
  bot.callbackQuery(/^slide:(prev|next|noop)$/, async (ctx) => {
    ...
    const index = action === "next" ? Math.min(session.currentIndex + 1, session.fileIds.length - 1) : Math.max(session.currentIndex - 1, 0);
    ...
    try {
      await ctx.api.raw.editMessageMedia({ inline_message_id: id, media: { type: "photo", media: session.fileIds[index]!, caption: resultCaption(session.lang, session.sourceLink), parse_mode: "HTML" }, reply_markup: navigationKeyboard(index, session.fileIds.length, session.likes, session.views) });
  ...
  bot.callbackQuery(/^sr:(\d+):(.+)$/, async (ctx) => {
    ...
    try {
      ...
      if (findInstagramUrl(link)) {
        ...
        files = fileIds(await service.stageInstagram(extraction, link, identity));
      } else {
        const extraction = await ctx.scrap.extractTikTok(link);
        files = fileIds(await service.stageTikTok(extraction, link, identity));
        likes = extraction.likes; views = extraction.views;
      }
      const keyboard = createInlineSlideshow(ctx.api, id, files, lang, link, identity, likes, views);
      const session = sessions.get(id)!;
      session.currentIndex = Math.max(0, Math.min(saved, files.length - 1));
      await ctx.api.raw.editMessageMedia({ inline_message_id: id, media: { type: "photo", media: files[session.currentIndex]!, caption: resultCaption(lang, link), parse_mode: "HTML" }, reply_markup: navigationKeyboard(session.currentIndex, files.length, likes, views) });
      await ctx.answerCallbackQuery();
...
function fileIds(result: Awaited<ReturnType<DeliveryService["stageTikTok"]>>): string[] {
  return allMessages(result).map(fileIdFromMessage).filter((value): value is string => !!value);
}
...
async function expire(api: Api, id: string): Promise<void> {
  ...
  try { await api.raw.editMessageReplyMarkup({ inline_message_id: id, reply_markup: expiredKeyboard(session.currentIndex, session.fileIds.length, session.sourceLink, session.likes, session.views) }); } catch { /* inline message gone */ }
}
